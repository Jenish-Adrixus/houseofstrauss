...
class WC_Checkout {
		...
		$this->fields = array(
			...
			'order'    => array(
				'order_comments' => array(
					'type'        => 'textarea',
					'class'       => array( 'notes' ),
					'label'       => __( 'BESTELLHINWEISE', 'woocommerce' ),
					'placeholder' => esc_attr__(
						'Anmerkungen zu Ihrer Bestellung, z. B. besondere Hinweise zur Lieferung.',
						'woocommerce'
					),
				),
			),
		...
		foreach ( $this->get_checkout_fields() as $fieldset_key => $fieldset ) {
			...
			foreach ( $fieldset as $key => $field ) {
				...
				if ( $validate_fieldset && $required && '' === $data[ $key ] ) {
					/* translators: %s: field name */
					$errors->add( $key . '_required', apply_filters( 'woocommerce_checkout_required_field_notice', sprintf( __( '%s ist ein Pflichtfeld.', 'woocommerce' ), '<strong>' . esc_html( $field_label ) . '</strong>' ), $field_label, $key ), array( 'id' => $key ) );
				}
			}
		}
	}
	...
	protected function validate_checkout( &$data, &$errors ) {
		...
		if ( empty( $data['woocommerce_checkout_update_totals'] ) && empty( $data['terms'] ) && ! empty( $data['terms-field'] ) ) {
			$errors->add( 'terms', __( 'Bitte lesen und akzeptieren Sie die allgemeinen Geschäftsbedingungen, um mit Ihrer Bestellung fortzufahren.', 'woocommerce' ) );
		}
		...
		if ( WC()->cart->needs_shipping() ) {
			...
			if ( empty( $shipping_country ) ) {
				$errors->add( 'shipping', __( 'Bitte geben Sie eine Adresse ein, um fortzufahren.', 'woocommerce' ) );
				...
				if ( WC()->countries->country_exists( $shipping_country ) ) {
					...
					$errors->add( 'shipping', sprintf( __( 'Leider <strong>wir versenden nicht %s</strong>. Bitte geben Sie eine alternative Lieferadresse an.', 'woocommerce' ), WC()->countries->shipping_to_prefix() . ' ' . $shipping_country ) );
				}
			} else {
				...
				foreach ( WC()->shipping()->get_packages() as $i => $package ) {
					if ( ! isset( $chosen_shipping_methods[ $i ], $package['rates'][ $chosen_shipping_methods[ $i ] ] ) ) {
						$errors->add( 'shipping', __( 'Es wurde keine Versandart ausgewählt. Bitte überprüfen Sie Ihre Adresse, oder kontaktieren Sie uns, wenn Sie Hilfe benötigen.', 'woocommerce' ) );
					}
				}
			}
		}
