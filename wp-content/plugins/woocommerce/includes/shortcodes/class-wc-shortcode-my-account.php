...
class WC_Shortcode_My_Account {
	...
	public static function output( $atts ) {
		...
		if ( ! is_user_logged_in() || isset( $wp->query_vars['lost-password'] ) ) {
			...
			if ( ! empty( $_GET['password-reset'] ) ) { // WPCS: input var ok, CSRF ok.
				wc_add_notice( __( 'Ihr Passwort wurde erfolgreich zurückgesetzt.', 'woocommerce' ) );
			}
			...
			if ( isset( $wp->query_vars['customer-logout'] ) ) {
				...
				wc_add_notice( sprintf( __( 'Sind Sie sicher, dass Sie sich abmelden möchten? <a href="%s">Bestätigen und abmelden</a>', 'woocommerce' ), wc_logout_url() ) );
			}
	...
	public static function retrieve_password() {
		...
		if ( empty( $login ) ) {
			...
			wc_add_notice( __( 'Geben Sie einen Benutzernamen oder eine E-Mail-Adresse ein.', 'woocommerce' ), 'error' );
		...
		if ( ! $user_data ) {
			wc_add_notice( __( 'Ungültiger Benutzername oder E-Mail.', 'woocommerce' ), 'error' );
		...
		if ( ! $allow ) {
			...
			wc_add_notice( __( 'Passwort zurücksetzen ist für diesen Benutzer nicht erlaubt', 'woocommerce' ), 'error' );
	...
	public static function check_password_reset_key( $key, $login ) {
		...
		if ( is_wp_error( $user ) ) {
			wc_add_notice( __( 'Dieser Schlüssel ist ungültig oder wurde bereits verwendet. Bitte setzen Sie Ihr Passwort bei Bedarf erneut zurück.', 'woocommerce' ), 'error' );
			return false;
		}
