...
function wc_get_account_menu_items() {
	...
	$items = array(
		...
		'downloads'       => __( 'Herunterladen', 'woocommerce' ),
		'edit-address'    => _n( 'Adresse', 'Addresses', ( 1 + (int) wc_shipping_enabled() ), 'woocommerce' ),
		'payment-methods' => __( 'Zahlungsmöglichkeiten', 'woocommerce' ),
...
function wc_get_account_orders_columns() {
	...
	return apply_filters(
		...
		array(
			'order-number'  => __( 'Bestellung', 'woocommerce' ),
			'order-date'    => __( 'Datum', 'woocommerce' ),
			'order-status'  => __( 'Status', 'woocommerce' ),
			'order-total'   => __( 'Gesamt', 'woocommerce' ),
			'order-actions' => __( 'Aktionen', 'woocommerce' ),
		)
	);
}
...
function wc_get_account_downloads_columns() {
	$columns = apply_filters(
		...
		array(
			'download-product'   => __( 'Produkt', 'woocommerce' ),
			'download-remaining' => __( 'Verbleibende Herunterladen', 'woocommerce' ),
			'download-expires'   => __( 'Abgelaufen', 'woocommerce' ),
			'download-file'      => __( 'Herunterladen', 'woocommerce' ),
			'download-actions'   => '&nbsp;',
		)
	);
...
function wc_get_account_orders_actions( $order ) {
	...
	$actions = array(
		'pay'    => array(
			...
			'name' => __( 'Bezahlen', 'woocommerce' ),
		),
		'view'   => array(
			'url'  => $order->get_view_order_url(),
			'name' => __( 'Anzeigen', 'woocommerce' ),
		),
		'cancel' => array(
			'url'  => $order->get_cancel_order_url( wc_get_page_permalink( 'myaccount' ) ),
			'name' => __( 'Abbrechen', 'woocommerce' ),
		),
	);
	...
	foreach ( $payment_tokens as $payment_token ) {
		...
		$list[ $type ][] = array(
			...
			'actions'    => array(
				'delete' => array(
					'url'  => $delete_url,
					'name' => esc_html__( 'Löschen', 'woocommerce' ),
				),
			),
		);
		...
		if ( ! $payment_token->is_default() ) {
			$list[ $type ][ $key ]['actions']['default'] = array(
				'url'  => $set_default_url,
				'name' => esc_html__( 'Standard machen', 'woocommerce' ),
			);
		}
