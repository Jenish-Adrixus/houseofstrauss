...
class WC_Form_Handler {
	...
	public static function redirect_reset_password_link() {
		if ( is_account_page() && isset( $_GET['key'] ) && ( isset( $_GET['id'] ) || isset( $_GET['login'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			if ( $logged_in_user_id && $logged_in_user_id !== $user_id ) {
				wc_add_notice( __( 'Dieser Schlüssel zum Zurücksetzen des Passworts ist für ein anderes Benutzerkonto. Bitte melden Sie sich ab und versuchen Sie es erneut.', 'woocommerce' ), 'error' );
				return;
			}
	...
	public static function save_address() {
		...
		foreach ( $address as $key => $field ) {
			...
			if ( ! empty( $field['required'] ) && empty( $value ) ) {
				...
				wc_add_notice( sprintf( __( '%s ist ein Pflichtfeld.', 'woocommerce' ), $field['label'] ), 'error', array( 'id' => $key ) );
			}
						...
						switch ( $rule ) {
							case 'postcode':
								...
								if ( '' !== $value && ! WC_Validation::is_postcode( $value, $country ) ) {
									switch ( $country ) {
										case 'IE':
											$postcode_validation_notice = __( 'Bitte geben Sie einen gültigen Eircode ein.', 'woocommerce' );
											break;
										default:
											$postcode_validation_notice = __( 'Bitte geben Sie eine gültige Postleitzahl ein.', 'woocommerce' );
									}
							...
							case 'phone':
								if ( '' !== $value && ! WC_Validation::is_phone( $value ) ) {
									...
									wc_add_notice( sprintf( __( '%s ist keine gültige Rufnummer.', 'woocommerce' ), '<strong>' . $field['label'] . '</strong>' ), 'error' );
								}
								break;
							case 'email':
								...
								if ( ! is_email( $value ) ) {
									...
									wc_add_notice( sprintf( __( '%s ist keine gültige E-Mail Adresse.', 'woocommerce' ), '<strong>' . $field['label'] . '</strong>' ), 'error' );
								}
								break;
						}
					}
	...
	public static function save_account_details() {
		...
		if ( is_email( $account_display_name ) ) {
			wc_add_notice( __( 'Der Anzeigename kann aus Gründen des Datenschutzes nicht in eine E-Mail-Adresse geändert werden.', 'woocommerce' ), 'error' );
		}
		...
		$required_fields = apply_filters(
			...
			array(
				'account_first_name'   => __( 'Vornamen', 'woocommerce' ),
				'account_last_name'    => __( 'Nachname', 'woocommerce' ),
				'account_display_name' => __( 'Anzeigename', 'woocommerce' ),
				'account_email'        => __( 'E-Mail Adresse', 'woocommerce' ),
			)
		);
		...
		foreach ( $required_fields as $field_key => $field_name ) {
			if ( empty( $_POST[ $field_key ] ) ) {
				/* translators: %s: Field name. */
				wc_add_notice( sprintf( __( '%s ist ein Pflichtfeld.', 'woocommerce' ), '<strong>' . esc_html( $field_name ) . '</strong>' ), 'error', array( 'id' => $field_key ) );
			}
		}
		...
		if ( $account_email ) {
			...
			if ( ! is_email( $account_email ) ) {
				wc_add_notice( __( 'Ungültige E-Mail.', 'woocommerce' ), 'error' );
			} elseif ( email_exists( $account_email ) && $account_email !== $current_user->user_email ) {
				wc_add_notice( __( 'Diese E-Mail Adresse ist bereits registriert.', 'woocommerce' ), 'error' );
			}
			$user->user_email = $account_email;
		}

		if ( ! empty( $pass_cur ) && empty( $pass1 ) && empty( $pass2 ) ) {
			wc_add_notice( __( 'Bitte füllen Sie alle Passwortfelder aus.', 'woocommerce' ), 'error' );
			$save_pass = false;
		} elseif ( ! empty( $pass1 ) && empty( $pass_cur ) ) {
			wc_add_notice( __( 'Bitte geben Sie Ihr aktuelles Passwort ein.', 'woocommerce' ), 'error' );
			$save_pass = false;
		} elseif ( ! empty( $pass1 ) && empty( $pass2 ) ) {
			wc_add_notice( __( 'Bitte geben Sie Ihr Passwort erneut ein.', 'woocommerce' ), 'error' );
			$save_pass = false;
		} elseif ( ( ! empty( $pass1 ) || ! empty( $pass2 ) ) && $pass1 !== $pass2 ) {
			wc_add_notice( __( 'Neue Kennwörter stimmen nicht überein.', 'woocommerce' ), 'error' );
			$save_pass = false;
		} elseif ( ! empty( $pass1 ) && ! wp_check_password( $pass_cur, $current_user->user_pass, $current_user->ID ) ) {
			wc_add_notice( __( 'Ihr aktuelles Passwort ist falsch.', 'woocommerce' ), 'error' );
			$save_pass = false;
		}
		...
		if ( wc_notice_count( 'error' ) === 0 ) {
			...
			wc_add_notice( __( 'Kontodaten erfolgreich geändert.', 'woocommerce' ) );
		...
		if ( isset( $_POST['woocommerce_pay'], $_GET['key'] ) ) {
			...
			if ( $order_id === $order->get_id() && hash_equals( $order->get_order_key(), $order_key ) && $order->needs_payment() ) {
				...
				if ( ! empty( $_POST['terms-field'] ) && empty( $_POST['terms'] ) ) {
					wc_add_notice( __( 'Bitte lesen und akzeptieren Sie die Allgemeinen Geschäftsbedingungen, um mit Ihrer Bestellung fortzufahren.', 'woocommerce' ), 'error' );
					return;
				}
				...
				if ( $order->needs_payment() ) {
					try {
						...
						if ( ! $payment_method_id ) {
							throw new Exception( __( 'Ungültige Zahlungsmethode.', 'woocommerce' ) );
						}
		...
		if ( isset( $_POST['woocommerce_add_payment_method'], $_POST['payment_method'] ) ) {
			...
			if ( WC_Rate_Limiter::retried_too_soon( $rate_limit_id ) ) {
				wc_add_notice(
					sprintf(
						...
						_n(
							'Sie können eine neue Zahlungsmethode nicht so schnell nach der vorherigen hinzufügen. Bitte warten Sie für %d Sekunde.',
							'Sie können eine neue Zahlungsmethode nicht so schnell nach der vorherigen hinzufügen. Bitte warten Sie für %d Sekunde.',
							$delay,
							'woocommerce'
						),
			...
			if ( isset( $available_gateways[ $payment_method_id ] ) ) {
				...
				if ( ! $gateway->supports( 'add_payment_method' ) && ! $gateway->supports( 'tokenization' ) ) {
					wc_add_notice( __( 'Ungültiges Zahlungsgateway.', 'woocommerce' ), 'error' );
					return;
				}
				...
				if ( 'success' === $result['result'] ) {
					wc_add_notice( __( 'Zahlungsmethode erfolgreich hinzugefügt.', 'woocommerce' ) );
				}

				if ( 'failure' === $result['result'] ) {
					wc_add_notice( __( 'Sie können keine Zahlungsmethode zu Ihrem Konto hinzufügen.', 'woocommerce' ), 'error' );
				}
	...
	public static function delete_payment_method_action() {
		...
		if ( isset( $wp->query_vars['delete-payment-method'] ) ) {
			...
			if ( is_null( $token ) || get_current_user_id() !== $token->get_user_id() || ! isset( $_REQUEST['_wpnonce'] ) || false === wp_verify_nonce( wp_unslash( $_REQUEST['_wpnonce'] ), 'delete-payment-method-' . $token_id ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				wc_add_notice( __( 'Ungültige Zahlungsmethode.', 'woocommerce' ), 'error' );
			} else {
				WC_Payment_Tokens::delete( $token_id );
				wc_add_notice( __( 'Zahlungsmethode gelöscht.', 'woocommerce' ) );
			}
	...
	public static function set_default_payment_method_action() {
		...
		if ( isset( $wp->query_vars['set-default-payment-method'] ) ) {
			...
			if ( is_null( $token ) || get_current_user_id() !== $token->get_user_id() || ! isset( $_REQUEST['_wpnonce'] ) || false === wp_verify_nonce( wp_unslash( $_REQUEST['_wpnonce'] ), 'set-default-payment-method-' . $token_id ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				...
				wc_add_notice( __( 'Diese Zahlungsmethode wurde erfolgreich als Ihre Standardmethode festgelegt.', 'woocommerce' ) );
			}
	...
	public static function cancel_order() {
		if (
			...
			if ( $user_can_cancel && $order_can_cancel && $order->get_id() === $order_id && hash_equals( $order->get_order_key(), $order_key ) ) {
				...
				wc_add_notice( __( 'Ihre Bestellung kann nicht mehr storniert werden. Bitte kontaktieren Sie uns, wenn Sie Hilfe benötigen.', 'woocommerce' ), 'error' );
			} else {
				wc_add_notice( __( 'Ungültige Bestellung.', 'woocommerce' ), 'error' );
			}
	...
	private static function add_to_cart_handler_grouped( $product_id ) {
		...
		if ( ! empty( $items ) ) {
			...
			if ( ! $was_added_to_cart && ! $quantity_set ) {
				wc_add_notice( __( 'Bitte wählen Sie die Anzahl der Artikel, die Sie in Ihren Warenkorb legen möchten&hellip;', 'woocommerce' ), 'error' );
			...
			wc_add_notice( __( 'Bitte wählen Sie ein Produkt aus, das Sie in Ihren Warenkorb legen möchten&hellip;', 'woocommerce' ), 'error' );
		}
		return false;
	}
	...
	private static function add_to_cart_handler_variable( $product_id ) {
		...
		if ( empty( $variation_id ) && $product && $product->is_type( 'variable' ) ) {
			...
			wc_add_notice( sprintf( __( 'Bitte wählen Sie die Produktoptionen unter <a href="%1$s" title="%2$s">%2$s</a>.', 'woocommerce' ), esc_url( get_permalink( $product_id ) ), esc_html( $product->get_name() ) ), 'error' );
		...
		if ( isset( $_POST['login'], $_POST['username'], $_POST['password'] ) && $valid_nonce ) {
			...
			try {
				...
				if ( $validation_error->get_error_code() ) {
					throw new Exception( '<strong>' . __( 'Fehler:', 'woocommerce' ) . '</strong> ' . $validation_error->get_error_message() );
				}

				if ( empty( $creds['user_login'] ) ) {
					throw new Exception( '<strong>' . __( 'Fehler:', 'woocommerce' ) . '</strong> ' . __( 'Der Benutzername ist erforderlich.', 'woocommerce' ) );
				}
	...
	public static function process_reset_password() {
		...
		if ( $user instanceof WP_User ) {
			if ( empty( $posted_fields['password_1'] ) ) {
				wc_add_notice( __( 'Bitte geben Sie Ihr Passwort ein.', 'woocommerce' ), 'error' );
			}

			if ( $posted_fields['password_1'] !== $posted_fields['password_2'] ) {
				wc_add_notice( __( 'Die Kennwörter stimmen nicht überein.', 'woocommerce' ), 'error' );
			}
			...
			try {
				...
				if ( 1 === count( $validation_errors ) ) {
					...
					foreach ( $validation_errors as $message ) {
						wc_add_notice( '<strong>' . __( 'Fehler:', 'woocommerce' ) . '</strong> ' . $message, 'error' );
					}
					throw new Exception();
				}
				...
				if ( 'yes' === get_option( 'woocommerce_registration_generate_password' ) ) {
					wc_add_notice( __( 'Ihr Konto wurde erfolgreich erstellt und ein Passwort wurde an Ihre E-Mail-Adresse gesendet.', 'woocommerce' ) );
				} else {
					wc_add_notice( __( 'Ihr Konto wurde erfolgreich erstellt. Ihre Anmeldedaten wurden an Ihre E-Mail-Adresse gesendet.', 'woocommerce' ) );
				}
				...
				if ( $e->getMessage() ) {
					wc_add_notice( '<strong>' . __( 'Fehler:', 'woocommerce' ) . '</strong> ' . $e->getMessage(), 'error' );
				}
			}
		}
	}
