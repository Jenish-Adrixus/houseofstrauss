		...
		if ( empty( $request_payment_method ) ) {
			if ( $requires_payment_method ) {
				throw new RouteException(
					...
					__( 'Keine Zahlungsmethode angegeben.', 'woocommerce' ),
					400
				);
			}
			return null;
		...
		if ( ! isset( $available_gateways[ $request_payment_method ] ) ) {
			...
			throw new RouteException(
				...
				sprintf(
					...
					__( '%s ist für diese Bestellung nicht verfügbar - bitte wählen Sie eine andere Zahlungsmethode', 'woocommerce' ),
			...
			switch ( $error->getMessage() ) {
				case 'registration-error-invalid-email':
					throw new RouteException(
						...
						__( 'Ungültige E-Mail.', 'woocommerce' ),
						400
					);
				case 'registration-error-email-exists':
					throw new RouteException(
						'registration-error-email-exists',
						__( 'Es ist bereits ein Konto mit Ihrer E-Mail-Adresse registriert. Bitte melden Sie sich an, bevor Sie fortfahren.', 'woocommerce' ),
						400
					);
			}
		}
