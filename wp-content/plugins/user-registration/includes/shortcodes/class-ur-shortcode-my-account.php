...
class UR_Shortcode_My_Account {
	...
	public static function output( $atts ) {
		...
		if ( ! is_user_logged_in() ) {
			...
			if ( ! empty( $is_password_resetted ) ) {
				ur_add_notice( __( 'Ihr Passwort wurde erfolgreich zurückgesetzt.', 'user-registration' ) );
			...
			if ( isset( $wp->query_vars['user-logout'] ) ) {
				...
				ur_add_notice( sprintf( __( 'Sind Sie sicher, dass Sie sich abmelden möchten? <a href="%s">Bestätigen und abmelden</a>', 'user-registration' ), ur_logout_url() ) );
			}
	...
	public static function edit_profile() {
		...
		if ( ! empty( $form_data_array ) ) {
			...
			echo '<h1>' . esc_html__( 'Keine Profildetails gefunden.', 'user-registration' ) . '</h1>';
		}
	}
	...
	public static function retrieve_password() {
		...
		if ( empty( $login ) ) {
			ur_add_notice( __( 'Geben Sie einen Benutzernamen oder eine E-Mail-Adresse ein.', 'user-registration' ), 'error' );
			return false;
		} else {
		...
		if ( ! $user_data ) {
			ur_add_notice( __( 'Ungültiger Benutzername oder E-Mail.', 'user-registration' ), 'error' );
			return false;
		}
		...
		if ( ! $allow ) {
			ur_add_notice( __( 'Passwort zurücksetzen ist für diesen Benutzer nicht erlaubt', 'user-registration' ), 'error' );
			return false;
		...
		if ( UR_Emailer::lost_password_email( $user_login, $user_data, $key ) == false ) {
			ur_add_notice( __( 'Die E-Mail konnte nicht gesendet werden. Wenden Sie sich an Ihren Website-Administrator. ', 'user-registration' ), 'error' );
			return false;
		}
