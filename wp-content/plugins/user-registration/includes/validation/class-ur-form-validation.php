...
class UR_Form_Validation extends UR_Validation {
		...
		if ( isset( $form_field_data[ $key ]->general_setting->field_name ) && $value == $form_field_data[ $key ]->general_setting->field_name ) {
			...
			if ( isset( $form_field_data[ $key ]->general_setting->required ) && ur_string_to_bool( $form_field_data[ $key ]->general_setting->required ) ) {
				...
				if ( isset( $form_field_data[ $key ]->advance_setting->field_visibility ) && 'edit_form' === $form_field_data[ $key ]->advance_setting->field_visibility ) {
					...
					$response = sprintf( __( '<strong>%s</strong> ist ein Pflichtfeld.', 'user-registration' ), $field_label );
					array_push( $this->response_array, $response );
				}
			}
		}
	...
	public function get_error_message( $error_code = '', $field_label = '' ) {
		$errors = array(
			'invalid_email'     => 'Bitte geben Sie eine gültige E-Mail für %s.',
			'invalid_url'       => 'Bitte geben Sie eine gültige Url für %s.',
			'invalid_date'      => 'Bitte geben Sie ein gültiges Datum für %s.',
			'empty_field'       => '%s ist ein Pflichtfeld.',
			'non_boolean_value' => 'Bitte geben Sie einen gültigen Wert für %s.',
			'non_numeric_data'  => 'Bitte geben Sie einen numerischen Wert für %s.',
		);
		...
		if ( in_array( $error_code, array_keys( $errors ), true ) ) {
			...
			return sprintf(
				'Der Wert, den Sie für <strong>%s</strong> eingegeben haben, ist ungültig.',
				$field_label
			);
		}
	}
	...
	public function validate_update_profile_AJAX( $form_fields, $form_data, $form_id ) {
		...
		if ( array_diff( $form_key_list, $request_form_keys ) ) {
			ur_add_notice( 'Einige Felder fehlen in dem übermittelten Formular. Bitte laden Sie die Seite neu.', 'error' );
			return;
		}
