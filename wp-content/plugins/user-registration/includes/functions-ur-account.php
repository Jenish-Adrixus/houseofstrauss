...
function ur_login_error_message( $error ) {
	...
	if ( is_int( $pos ) && isset( $_POST['username'] ) ) {  // phpcs:ignore WordPress.Security.NonceVerification
		...
		$error = sprintf( '<strong>' . __( 'Fehler:', 'user-registration' ) . '</strong>' . __( 'Das Passwort, das Sie für den Benutzernamen eingegeben haben %1$1s ist falsch. %2$2s', 'user-registration' ), sanitize_text_field( wp_unslash( $_POST['username'] ) ), "<a href='" . esc_url( wp_lostpassword_url() ) . "'>" . __( 'Haben Sie Ihr Passwort vergessen?', 'user-registration' ) . '</a>' ); // phpcs:ignore WordPress.Security.NonceVerification
	} elseif ( is_int( $pos2 ) && isset( $_POST['username'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
		/* translators: %s - Lost password URL */
		$error = sprintf( '<strong>' . __( 'Fehler:', 'user-registration' ) . '</strong>' . __( 'Ungültiger Benutzername. %1s', 'user-registration' ), "<a href='" . esc_url( wp_lostpassword_url() ) . "'>" . __( 'Haben Sie Ihr Passwort vergessen?', 'user-registration' ) . '</a>' ); // phpcs:ignore WordPress.Security.NonceVerification
	}
...
function ur_get_account_menu_items() {
	...
	$items = array(
		...
		'edit-profile'  => __( 'Profil', 'user-registration' ),
		'edit-password' => __( 'Passwort ändern', 'user-registration' ),
		'user-logout'   => __( 'Abmelden', 'user-registration' ),
	);
