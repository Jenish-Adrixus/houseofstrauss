...
class UR_Form_Handler {
	...
	public static function save_profile_details() {
		...
		if ( 0 === ur_notice_count( 'error' ) ) {
			...
			if ( $email_updated ) {
				...
				$user_email_update_message = sprintf( __( 'Ihre E-Mail Adresse wurde noch nicht aktualisiert. Bitte überprüfen Sie Ihren Posteingang unter <strong>%s</strong> um eine Bestätigungs-E-Mail zu erhalten.', 'user-registration' ), $pending_email );
	...
	public static function send_confirmation_email( $user, $new_email ) {
		...
		$to           = $new_email;
		$subject      = apply_filters( 'user_registration_email_change_email_subject', __( 'Bestätigen Sie Ihre E-Mail-Adressänderung', 'user-registration' ) );
		$message      = sprintf(
			/* translators: %1$s is the display name of the user, %2$s is the new email, %3$s is the confirmation link, %4$s is the blog name. */
			__(
				'Liebe %1$s,<br /><br />
				Sie haben kürzlich beantragt, die mit Ihrem Konto verknüpfte E-Mail-Adresse in %2$s.<br /><br />
				Um diese Änderung zu bestätigen, klicken Sie bitte auf den folgenden Link:<br />
				<a href="%3$s">%3$s</a><br /><br />
				Dieser Link wird nur für 24 Stunden aktiv sein. Wenn Sie diese Änderung nicht angefordert haben, ignorieren Sie bitte diese E-Mail oder kontaktieren Sie uns, um Hilfe zu erhalten.<br /><br />
	...
	public static function save_change_password() {
		...
		if ( empty( $pass_cur ) && empty( $pass1 ) && empty( $pass2 ) ) {
			ur_add_notice( __( 'Bitte füllen Sie alle Passwortfelder aus.', 'user-registration' ), 'error' );
			$save_pass = false;
		} elseif ( ! $bypass_current_password && empty( $pass_cur ) ) {
			ur_add_notice( __( 'Bitte geben Sie Ihr aktuelles Passwort ein.', 'user-registration' ), 'error' );
			$save_pass = false;
		} elseif ( empty( $pass1 ) ) {
			ur_add_notice( __( 'Bitte geben Sie Ihr neues Passwort ein.', 'user-registration' ), 'error' );
			$save_pass = false;
		} elseif ( empty( $pass2 ) ) {
			ur_add_notice( __( 'Bitte geben Sie Ihr Passwort erneut ein.', 'user-registration' ), 'error' );
			$save_pass = false;
		} elseif ( $pass1 !== $pass2 ) {
			ur_add_notice( __( 'Die neuen Kennwörter stimmen nicht überein.', 'user-registration' ), 'error' );
			$save_pass = false;
		} elseif ( ! $bypass_current_password && ! wp_check_password( $pass_cur, $current_user->user_pass, $current_user->ID ) ) {
			ur_add_notice( __( 'Ihr aktuelles Passwort ist falsch.', 'user-registration' ), 'error' );
			$save_pass = false;
		} elseif ( wp_check_password( $pass1, $current_user->user_pass, $current_user->ID ) && $current_user ) {
			ur_add_notice( __( 'Das neue Passwort darf nicht mit dem alten identisch sein', 'user-registration' ), 'error' );
			$save_pass = false;
		}
		...
		if ( ur_notice_count( 'error' ) === 0 ) {
			...
			ur_add_notice( __( 'Passwort erfolgreich geändert.', 'user-registration' ) );
	...
	public static function process_reset_password() {
		...
		if ( $user instanceof WP_User ) {
			if ( empty( $posted_fields['password_1'] ) ) {
				ur_add_notice( esc_html__( 'Bitte geben Sie Ihr Passwort ein.', 'user-registration' ), 'error' );
			}

			if ( $posted_fields['password_1'] !== $posted_fields['password_2'] ) {
				ur_add_notice( esc_html__( 'Die Kennwörter stimmen nicht überein.', 'user-registration' ), 'error' );
			}

			if ( wp_check_password( $posted_fields['password_1'], $user->user_pass, $user->ID ) ) {
				ur_add_notice( esc_html__( 'Das neue Passwort darf nicht mit dem alten identisch sein.', 'user-registration' ), 'error' );
			}
			$errors = new WP_Error();
		...
		if ( isset( $_REQUEST['action'] ) && 'confirmaction' === $_REQUEST['action'] ) {
			...
			if ( $request && in_array( $request->action_name, _wp_privacy_action_request_types(), true ) ) {
				if ( 'export_personal_data' === $request->action_name ) {
					$message = apply_filters( 'user_registration_export_personal_data_confirmation_message', __( 'Vielen Dank für die Bestätigung Ihrer Exportanfrage.', 'user-registration' ) );
				} elseif ( 'remove_personal_data' === $request->action_name ) {
					$message = apply_filters( 'user_registration_remove_personal_data_confirmation_message', __( 'Vielen Dank für die Bestätigung Ihres Löschungsantrags.', 'user-registration' ) );
				}
